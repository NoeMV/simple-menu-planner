<script setup>
    ...
    onBeforeMount(async ()=> {
        ...
        await menuStore.getParticipants(menuStore.menu.participants);
        await menuStore.getBlocked(menuStore.menu.blocked);
    });
    ...
    async function removeParticipant(id){
        ...
        await menuStore.updateMenu(props.id, {participants: participantList});
        await menuStore.getMenu(props.id);
        await menuStore.getRequests(menuStore.menu.requests);
        await menuStore.getParticipants(menuStore.menu.participants);
        await menuStore.getBlocked(menuStore.menu.blocked);
    ...
    <div class="mx-auto max-w-7xl px-10 h-full flex justify-center items-center">
        <div class="py-8 px-10 w-full flex flex-col justify-center items-center space-y-8 bg-white bg-opacity-50 backdrop-blur-lg drop-shadow-md rounded-lg shadow-lg">
            ...
            <span v-for="participant in menuStore.participants" :key="participant.id">
                ...
                <button v-if="userStore.user?.menus.find(e => e.code == props.id).creator == true && participant.id != userStore.user.id" @click="removeParticipant(participant.id)">eliminar</button>
            </span>
