        ...
        <div class="relative py-14 px-10 w-3/4 sm:w-fit flex flex-col justify-center bg-slate-100 rounded-xl shadow-xl">
            ...
            <form class="flex flex-col items-center justify-center w-full px-6 mt-8" @submit.prevent="handleJoin()">
                <div class="mb-6 relative w-full">
                    <input type="text" autocomplete="off" id="code" placeholder="code" v-model="code" class="peer mt-1 w-full bg-opacity-0 bg-slate-100 border-b-2 border-slate-600 px-0 py-2 placeholder:text-transparent focus:border-slate-800 focus:outline-none">
                    <label for="code" class="font-signika-negative pointer-events-none absolute top-0 left-0 origin-left -translate-y-1/2 transform text-sm text-slate-600 opacity-75 transition-all duration-100 ease-in-out peer-placeholder-shown:top-1/2 peer-placeholder-shown:text-base peer-placeholder-shown:text-slate-600 peer-placeholder-shown:font-signika-negative peer-focus:top-0 peer-focus:pl-0 peer-focus:text-sm peer-focus:text-slate-800">
                        Código del menu
                    </label>
                </div>
