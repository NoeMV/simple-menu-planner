...
<template>
    <div class="mx-auto max-w-7xl px-16 h-full flex justify-center items-center">
        <div class="py-14 px-6 w-fit flex flex-col justify-center bg-slate-100 rounded-xl shadow-xl">
            ...
            <form class="flex flex-col items-center justify-center w-full px-6 mt-8" @submit.prevent="userStore.register(name, email, password)">
                <div class="mb-6 relative w-full">
                    <input required type="name" autocomplete="off" id="name" placeholder="name" v-model="name" class="peer mt-0 w-full bg-white border border-slate-300 px-2.5 py-1 rounded-xl placeholder:text-transparent focus:border-sky-600 focus:ring-2 focus:ring-offset-0 focus:ring-offset-sky-600 focus:ring-opacity-60">
                ...
                <router-link :to="{name: 'Login'}" class="font-signika-negative font-normal text-sm text-center text-slate-500 underline hover:text-slate-700">¿Tienes una cuenta? Inicia sesión</router-link>
            </form>
